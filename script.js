...
const weeklyData = {
  ...
  14: {
    ...
    content: `
      ...
      <h3>🤝 與 AI 協作除錯</h3>
      <p>在 Raspberry Pi 地震儀實作中，我不是一次就得到正確結果，而是透過不斷上傳波形、STA/LTA、頻譜、PGA 與位移趨勢圖，與 AI 一起判讀「為什麼沒晃動也被判定為地震」、「為什麼蜂鳴器太敏感」、「為什麼位移圖不合理」。這個過程讓我學會從圖表反推程式問題，並加入高通濾波、基線校正、事件門檻、STA/LTA 輔助確認與蜂鳴器延遲規則。</p>
      <div class="modal-links">
        <a href="#ai-collab" onclick="closeModal()" class="modal-link-btn btn-web">🤝 前往 AI 協作與試錯修正區</a>
      </div>
  ...
  15: {
    ...
    content: `
      ...
      <h3>📋 成果</h3>
      <p>完成 LaTeX 排版技能學習，包含公式編輯、圖表引用、參考文獻管理、雙欄排版等學術報告必備技巧。為了讓讀者可以直接檢視我的 Overleaf 實作成果，我將本週完成的兩份 PDF 放入下方：一份是 <strong>report 形式</strong> 的完整書面報告，一份是 <strong>presentation 形式</strong> 的期末簡報版本。</p>

      <div class="overleaf-pdf-grid">
        <a class="overleaf-pdf-card" href="overleaf_report_form.pdf" target="_blank" rel="noopener">
          <div class="overleaf-pdf-icon">📘</div>
          <div>
            <h4>Report 形式｜完整書面報告</h4>
            <p>以學術報告格式整理 16 週地震學學習歷程，包含 AI 輔助學習、SmartSolo 震測、NCREE 參訪、震源機制、IoT 地震儀與期末反思。</p>
            <span>點擊開啟 PDF ↗</span>
          </div>
        </a>
        <a class="overleaf-pdf-card" href="overleaf_presentation_form.pdf" target="_blank" rel="noopener">
          <div class="overleaf-pdf-icon">🎞️</div>
          <div>
            <h4>Presentation 形式｜期末簡報版</h4>
            <p>以簡報格式濃縮呈現一學期成果，強調時間軸、重點圖像、互動網頁與 Raspberry Pi 地震儀實作的報告邏輯。</p>
            <span>點擊開啟 PDF ↗</span>
          </div>
        </a>
      </div>
      ...
      <div class="modal-links">
        <a href="overleaf_report_form.pdf" target="_blank" rel="noopener" class="modal-link-btn btn-pdf">📄 查看 Report 形式 PDF</a>
        <a href="overleaf_presentation_form.pdf" target="_blank" rel="noopener" class="modal-link-btn btn-pdf">📊 查看 Presentation 形式 PDF</a>
      </div>
    `
  },
  16: {
